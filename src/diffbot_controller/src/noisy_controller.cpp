                                  ...
                                  , theta_(0.0)
{
    ...
    declare_parameter("wheel_separation", 0.1402203698837279);
...
{
    // Simulated sensor noise: perturb each wheel's raw encoder reading with
    // independent gaussian noise before computing odometry, so this node's
    // output mimics an imperfect real encoder instead of the ground truth
    // used by simple_controller.
    ...
    // Store the raw (noise-free) positions for next iteration's delta,
    // so noise doesn't accumulate across callbacks
