...
| `diffbot_localization` | Config/Launch/C++ | Sensor fusion — `imu_republisher` re-frames `/imu/out` to `base_footprint_ekf`; `robot_localization` EKF (`ekf.yaml`) fuses it with noisy wheel odometry (`odom_noisy`). Also `global_localization.launch.py` — `nav2_map_server` + `nav2_lifecycle_manager`, serves a static map from `diffbot_mapping` on `/map` (manual/standalone, not in bringup yet — groundwork for AMCL) |
| `diffbot_mapping` | C++/Data | `mapping_with_known_poses` — hand-built log-odds occupancy grid mapper (Bresenham + inverse sensor model) from `/scan` + TF, run manually; pre-built maps (`small_house`, `small_warehouse`) for `nav2_map_server` |
